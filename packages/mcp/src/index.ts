...
server.registerTool(listWireframes.name, { description: listWireframes.description }, () =>
  listWireframes.execute(),
)

server.registerTool(
  getWireframeJson.name,
  { description: getWireframeJson.description, inputSchema: getWireframeJson.schema },
  (args) => getWireframeJson.execute(args, service),
)

server.registerTool(
  getWireframePages.name,
  { description: getWireframePages.description, inputSchema: getWireframePages.schema },
  (args) => getWireframePages.execute(args, service),
)

server.registerTool(
  captureWireframe.name,
  { description: captureWireframe.description, inputSchema: captureWireframe.schema },
  (args) => captureWireframe.execute(args, service),
)

server.registerTool(
  getWireframeAssets.name,
  { description: getWireframeAssets.description, inputSchema: getWireframeAssets.schema },
